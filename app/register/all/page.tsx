...
async function DailyRegisterSection({ userId, currentPage }: { userId: string; currentPage: number }) {
  const pageSize = 15;

  const [dbUser, allExercises, allFoods, allWeights] = await Promise.all([
    prisma.user.findUnique({ where: { id: userId } }),
    prisma.exercise.findMany({ where: { userId }, orderBy: { date: "desc" } }),
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-7xl">
          ...
          <Card className="mt-8 glass-card border-none overflow-hidden">
            ...
            <Suspense key={currentPage} fallback={<TableCardSkeleton rows={10} />}>
              <DailyRegisterSection userId={user.id} currentPage={currentPage} />
            </Suspense>
          </Card>
        </div>
      </main>
